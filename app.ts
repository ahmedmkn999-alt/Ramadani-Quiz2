...
interface Prize {
  ...
  readonly item: keyof PowerUps | null;
  readonly angle: number;
  readonly chance: number;
}

interface NightQuestionData {
  question: string;
  options: string[];
  correctIndex: number;
  points: number;
...
let myLogs: Record<number, number> = {};
let adminDay: number = 1;
let adminStatus: string = "closed";
let logsUnsubscribe: (() => void) | null = null;

window.myPowerups = { freeze: 0, fifty50: 0 } satisfies PowerUps;
...
window.showAlert = function (
  ...
  const box = document.getElementById("custom-alert") as HTMLElement | null;
  const titleEl = document.getElementById("alert-title") as HTMLElement | null;
  const msgEl = document.getElementById("alert-msg") as HTMLElement | null;
  const iconEl = document.getElementById("alert-icon") as HTMLElement | null;
...
window.closeCustomAlert = function (): void {
  const box = document.getElementById("custom-alert") as HTMLElement | null;
...
function initFirebaseData(): void {
  ...
  db.collection("users")
    .doc(currentUser.id)
    .onSnapshot((doc: firebase.firestore.DocumentSnapshot): void => {
      if (doc.exists) {
        const d = doc.data() as Partial<User>;
        ...
        if (scoreEl) scoreEl.innerText = String(d.score ?? 0);
        if (nameEl) nameEl.innerText = d.name ?? "بطل";
        if (groupEl) groupEl.innerText = d.group ?? "";

        window.myPowerups = d.powerups ?? { freeze: 0, fifty50: 0 };
        if (freezeEl) freezeEl.innerText = String(window.myPowerups.freeze);
        if (fiftyEl) fiftyEl.innerText = String(window.myPowerups.fifty50);
      }
    });
  ...
  db.collection("settings")
    ...
    .onSnapshot((doc: firebase.firestore.DocumentSnapshot): void => {
      if (doc.exists) {
        const data = doc.data() as { currentDay?: number; status?: string };
...
function updateLogs(): void {
  ...
  logsUnsubscribe = db
    ...
    .onSnapshot((snap: firebase.firestore.QuerySnapshot): void => {
      myLogs = {};
      snap.forEach((d: firebase.firestore.QueryDocumentSnapshot): void => {
        const log = d.data() as unknown as GameLog;
...
function fetchLeaderboard(): void {
  ...
  db.collection("users")
    ...
    .get()
    .then((snap: firebase.firestore.QuerySnapshot): void => {
      const list: User[] = [];
      snap.forEach((d: firebase.firestore.QueryDocumentSnapshot): void => {
        list.push(d.data() as unknown as User);
      });
...
window.logoutUser = function (): void {
  localStorage.removeItem("currentUser");
  window.location.replace("index.html");
};
...
let nightQuestionData: NightQuestionData | null = null;

function initNightQuestion(): void {
  if (!db || !currentUser) return;

  const today = new Date();
  const dayKey = `night_${today.getFullYear()}_${today.getMonth() + 1}_${today.getDate()}`;
  const nqLog: NightQuestionLog = JSON.parse(
    localStorage.getItem(`nqLog_${currentUser.id}`) ?? "{}"
  );
  ...
  db.collection("settings")
    ...
    .onSnapshot((doc: firebase.firestore.DocumentSnapshot): void => {
      ...
      const data = doc.data() as unknown as NightQuestionData;
...
window.openNightQuestion = function (): void {
  ...
  const data: NightQuestionData = nightQuestionData;
  const pts = data.points ?? 10;
  const prizeEl = document.getElementById("nq-prize-pts");
  if (prizeEl) prizeEl.textContent = `+${pts}`;

  const arabicLetters = ["أ", "ب", "ج", "د"] as const;
...
window.handleNightAnswer = function (selectedIdx: number): void {
  if (!nightQuestionData || !currentUser || !db) return;
  ...
  document.querySelectorAll<HTMLButtonElement>(".nq-opt-btn").forEach((btn, idx) => {
...
window.closeNightQuestion = function (): void {
  const overlay = document.getElementById("nq-overlay");
...
function setupWheelTimer(): void {
  if (!currentUser) return;

  const key = `wheelData_${currentUser.id}`;
  let wheelData: WheelData = JSON.parse(localStorage.getItem(key) ?? "{}") as WheelData;
  ...
  if (wheelData.lastLogin) {
    ...
    const diffDays = Math.floor(Math.abs(today.getTime() - last.getTime()) / 86_400_000);
  ...
  if (badge) badge.innerText = `${wheelData.streak}/3`;
}

window.openSpinWheel = function (): void {
  if (!currentUser) return;

  const overlay = document.getElementById("spin-overlay") as HTMLElement | null;
  if (overlay) overlay.style.display = "flex";

  const wheelData: WheelData = JSON.parse(
    localStorage.getItem(`wheelData_${currentUser.id}`) ?? "{}"
  ) as WheelData;
...
window.closeSpinWheel = function (): void {
...
window.spinWheel = function (): void {
  if (isSpinning || !currentUser || !db) return;
  isSpinning = true;
  ...
  setTimeout((): void => {
    ...
    const key = `wheelData_${currentUser!.id}`;
    const wheelData: WheelData = JSON.parse(localStorage.getItem(key) ?? "{}") as WheelData;
    wheelData.streak = 0;
    localStorage.setItem(key, JSON.stringify(wheelData));
    ...
    if (prize.text === "حظ أوفر") {
      ...
      if (window.confetti) void window.confetti({ particleCount: 200, spread: 100, origin: { y: 0.5 } });
    ...
    setTimeout(() => location.reload(), 3_500);
  }, 4_000);
};
